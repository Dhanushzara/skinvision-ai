...
export default function ResultScreen() {
  ...
  // Safe JSON parse — prevents crash if data param is malformed or missing
  let scan: Partial<ScanResult> = {};
  try {
    scan = JSON.parse(data ?? '{}') as Partial<ScanResult>;
  } catch {
    scan = {};
  }

  // Guard: ensure nested objects are never null (prevents crash in render)
  if (!scan.abcde_scores)      scan.abcde_scores      = { asymmetry: 0, border: 0, color: 0, diameter: 0, evolution: 0 };
  if (!scan.all_probabilities)  scan.all_probabilities  = {};
  if (!scan.doctor_suggestion)  scan.doctor_suggestion  = { urgency_level: 'NONE', urgency_emoji: '✅', message: '', color: '#10B981' };
  ...
  const handleDownloadReport = async () => {
    ...
    const abcde = scan.abcde_scores!;
    const abcdeLines = Object.entries(abcde)
      .map(([k, v]) => `   ${k.charAt(0).toUpperCase() + k.slice(1).padEnd(12)}: ${v.toFixed(1)}/10`)
      .join('\n');

    const probLines = Object.entries(scan.all_probabilities!)
      .sort(([, a], [, b]) => b - a)
      .map(([cls, pct]) => `   ${cls.replace(/_/g, ' ').padEnd(16)}: ${pct.toFixed(1)}%`)
      .join('\n');

    const doctor = scan.doctor_suggestion;
    const doctorBlock = (doctor && doctor.urgency_level !== 'NONE')
      ? `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nDOCTOR RECOMMENDATION\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n   Urgency   : ${doctor.urgency_level}\n   Consult   : ${doctor.doctor_type ?? 'Dermatologist'}\n   Department: ${doctor.department ?? 'Dermatology'}\n   Timeline  : ${doctor.timeline ?? 'As advised'}\n   Note      : ${doctor.message}\n`
      : '';

    const report = `
╔══════════════════════════════════════╗
║       SKINVISION AI — REPORT        ║
╚══════════════════════════════════════╝

  Date       : ${dateStr}
  Scan ID    : ${scan.id ?? 'N/A'}
  Location   : ${scan.body_location ?? 'Not specified'}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DIAGNOSIS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Result     : ${scan.label ?? 'Unknown'}
  Confidence : ${scan.confidence ?? 0}%
  Risk Level : ${scan.risk_level ?? 'N/A'} (${scan.risk_score ?? 0}/100)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DISCLAIMER: This is an AI-generated report and NOT a substitute for
professional medical diagnosis. Always consult a qualified dermatologist
for any skin concerns.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Generated by SkinVision AI v3.0
`.trim();

    try {
      await Share.share({
        message: report,
        title: `SkinVision Report — ${scan.label ?? 'Scan'}`,
      });
    } catch {
      Alert.alert('Error', 'Could not share report. Please try again.');
    }
  };
  ...
  return (
    <ScrollView style={s.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
      ...
      <FadeIn delay={365}>
        <Section title="Hospital Integration (FHIR R4)" icon="business" defaultOpen={false}>
          <Text style={s.teleSubtitle}>
            Share a FHIR R4-compliant report with any hospital EMR system (Epic, Cerner) or rural clinic.
          </Text>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (!scan.id) { Alert.alert('Error', 'Scan ID not found.'); return; }
              Linking.openURL(`${API_BASE}/api/fhir/report/${scan.id}`);
            }}
          >
            <View style={[hs.fhirIcon, { backgroundColor: '#2563EB22' }]}>
              <Ionicons name="code-slash" size={18} color="#2563EB" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={hs.fhirBtnTxt}>View FHIR R4 Report</Text>
              <Text style={hs.fhirBtnSub}>HL7 FHIR • SNOMED CT • LOINC codes</Text>
            </View>
          ...
          <TouchableOpacity
            style={[hs.fhirBtn, { marginTop: 8 }]}
            activeOpacity={0.85}
            onPress={() => {
              if (!scan.id) { Alert.alert('Error', 'Scan ID not found.'); return; }
              Linking.openURL(`${API_BASE}/api/hospital/report/${scan.id}`);
            }}
          >
            <View style={[hs.fhirIcon, { backgroundColor: '#10B98122' }]}>
              <Ionicons name="document-text" size={18} color="#10B981" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={hs.fhirBtnTxt}>Clinical Summary Report</Text>
              <Text style={hs.fhirBtnSub}>Structured format for doctors & nurses</Text>
            </View>
            <Ionicons name="open-outline" size={16} color="#94A3B8" />
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              if (!scan.id) return;
              await Share.share({
                message: `SkinVision AI — FHIR Report\nScan: ${scan.id}\nDiagnosis: ${scan.label}\nRisk: ${scan.risk_level}\n\nFHIR Report: ${API_BASE}/api/fhir/report/${scan.id}\nClinical Summary: ${API_BASE}/api/hospital/report/${scan.id}`,
                title: 'Share FHIR Report with Hospital',
              });
            }}
          >
            <View style={[hs.fhirIcon, { backgroundColor: '#7C3AED22' }]}>
              <Ionicons name="share-social" size={18} color="#7C3AED" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={hs.fhirBtnTxt}>Send to Hospital / Clinic</Text>
              <Text style={hs.fhirBtnSub}>Share report URL via WhatsApp, Email, SMS</Text>
            </View>
            <Ionicons name="chevron-forward" size={16} color="#94A3B8" />
          </TouchableOpacity>

          <View style={hs.disclaimer}>
            <Ionicons name="information-circle" size={13} color="#64748B" />
            <Text style={hs.disclaimerTxt}>
              FHIR R4 standard • SNOMED CT clinical codes • Compatible with Epic, Cerner, OpenMRS & rural clinic systems
            </Text>
          </View>
        </Section>
      </FadeIn>
      ...
      <FadeIn delay={380} style={{ marginHorizontal: 16, marginBottom: 10 }}>
        <TouchableOpacity onPress={handleDownloadReport} activeOpacity={0.85}>
          <LinearGradient
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.exportBtnTxt}>Download Report</Text>
              <Text style={s.exportBtnSub}>Save to Notes, Email or WhatsApp</Text>
            </View>
...
const s = StyleSheet.create({
  ...
  exportBtn:    { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 16, paddingHorizontal: 18, borderRadius: 16 },
  exportBtnTxt: { color: 'white', fontSize: 14, fontWeight: '700' },
  exportBtnSub: { color: 'rgba(255,255,255,0.55)', fontSize: 11, marginTop: 1 },
});

const hs = StyleSheet.create({
  fhirBtn:      { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#F8FAFC', borderRadius: 14, padding: 13, borderWidth: 1, borderColor: '#E2E8F0' },
  fhirIcon:     { width: 38, height: 38, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  fhirBtnTxt:   { fontSize: 13, fontWeight: '700', color: '#0F172A' },
  fhirBtnSub:   { fontSize: 10, color: '#64748B', marginTop: 1 },
  disclaimer:   { flexDirection: 'row', alignItems: 'flex-start', gap: 6, marginTop: 12, backgroundColor: '#F1F5F9', borderRadius: 10, padding: 10 },
  disclaimerTxt:{ flex: 1, fontSize: 10, color: '#64748B', lineHeight: 15 },
});
